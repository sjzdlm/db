...
func NewDb(constr string) *xorm.Engine {
	...
	if v, ok := dbPool[constr]; ok {
		...
		if con["dbtype"] == "sqlite" {
			...
			XX, err = xorm.NewEngine(con["dbtype"], con["uid"]+":"+con["pwd"]+"@tcp("+con["server"]+":"+con["port"]+")/"+con["dbname"]+"?charset=utf8mb4&multiStatements=true")
		}
		if err != nil {
			return nil
		}
...
func NewDbMysql(conn string, server, port, uid, pwd, dbname string) *xorm.Engine {
	...
	if v, ok := dbPool[conn]; ok {
		...
		var XX *xorm.Engine
		var err error
		XX, err = xorm.NewEngine("mysql", uid+":"+pwd+"@tcp("+server+":"+port+")/"+dbname+"?charset=utf8mb4&multiStatements=true") //allowMultiQueries=true
		if err != nil {
			return nil
		}
		dbPool[conn] = XX
